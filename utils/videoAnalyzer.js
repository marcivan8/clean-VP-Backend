function analyzeVideo({ title, description, transcript }) {
  ...
  // Hook detection
  const firstWords = transcript ? transcript.trim().split(/\s+/).slice(0, 10).join(" ") : "";
  ...
  // Score calculation per platform with a minimum base score of 20
  const platformScores = {};
  for (const [platform, config] of Object.entries(platforms)) {
    let score = 20; // minimum base score
  ...
  const bestPlatform = Object.entries(platformScores).reduce((best, current) => {
