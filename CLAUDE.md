...
STILL OPEN: the PREVIEW is not graded (R53's `PlaybackEngine` has no LUT sampling — it needs the `.cube` loaded as a texture and sampled in the fragment shader), and neither is the Revideo/Lambda path. So a graded export currently will NOT match what the editor shows — the same preview-vs-export divergence R14 and R53 both document. Say so in the UI or fix the preview before treating this as complete.
