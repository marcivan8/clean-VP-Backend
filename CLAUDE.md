...
Billing
  POST   /api/webhooks/polar          polarWebhook.js   public (HMAC verified)
  (webhook → setPlan() → supabase profiles.plan)
...
THE SUBTLE TRAP if you touch any of these: `usePeaks(assetId, gcsPath, proxyUrl)`'s `gcsPath` argument is ALSO the raw path (asset.gcsPath is set from the raw upload's GCS key, not the proxy's), and `utils/waveformPath.js`'s `deriveGcsPath()` checks `gcsPath` FIRST, unconditionally — `if (rawGcsPath) return rawGcsPath`. Nulling out `proxyUrl` alone while still passing `asset.gcsPath` does NOTHING; the explicit gcsPath still forces raw-file resolution regardless of what proxyUrl says. Both arguments must be suppressed together.
...
SETUP REQUIRED BEFORE THIS RUNS MEANINGFULLY (not yet done as of this writing): create the `DAST_TEST_EMAIL`/`DAST_TEST_PASSWORD` Supabase account by hand — free plan, no real projects/uploads on it, so a scan that pokes at its data can't leak or cost anything real — and add all five secrets (`STAGING_URL`, `STAGING_SUPABASE_URL`, `STAGING_SUPABASE_ANON_KEY`, `DAST_TEST_EMAIL`, `DAST_TEST_PASSWORD`) to the `staging` GitHub environment. Until those exist, the workflow still runs (cron/dispatch both fire) but silently falls back to an unauthenticated scan per the paragraph above — check the run's `::warning::` annotations to tell which mode a given scan ran in.
...
[BILLING]      routes/polarWebhook.js, middleware/usageGate.js, client/src/hooks/useUserPlan.js
