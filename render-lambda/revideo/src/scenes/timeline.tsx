...
export default makeScene2D('timeline', function* (view) {
    ...
    // ── Font injection ────────────────────────────────────────────────────────
    // fontFaceCSS contains @font-face declarations generated by FontInstaller
    // in the Lambda handler. Fonts are embedded as base64 data URLs so Chrome
    // loads them without any network request or file:// cross-origin issue.
    //
    // This block runs synchronously before any clips are added to the scene,
    // ensuring fonts are available when the first Txt node renders.
    const fontFaceCSSSignal = vars.get('fontFaceCSS', '');
    const fontFaceCSS: string = (typeof fontFaceCSSSignal === 'function'
        ? fontFaceCSSSignal()
        : fontFaceCSSSignal) as string;

    if (fontFaceCSS && typeof fontFaceCSS === 'string' && fontFaceCSS.length > 0) {
        try {
            const styleEl = document.createElement('style');
            styleEl.id = 'vibed-fonts';
            styleEl.textContent = fontFaceCSS;
            document.head.appendChild(styleEl);

            // Yield document.fonts.ready so the Revideo engine waits until
            // Chrome has parsed and registered all @font-face rules before
            // rendering the first frame. For base64 data URLs this resolves
            // in < 50ms — it's just CSS parsing, no network round-trip.
            yield document.fonts.ready;
            console.log('[timeline] Fonts injected and ready');
        } catch (fontErr) {
            // Non-fatal: if injection fails, Chrome renders with its built-in
            // sans-serif. The video still exports — just with the wrong typeface.
            console.warn('[timeline] Font injection failed (non-fatal):', fontErr);
        }
    }
